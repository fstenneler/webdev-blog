        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Article : <?= esc($commentList[0][0]->post_title); ?></h6>
            </div>
            <div class="card-body">
                ...
                <?php foreach($commentList[0] as $commentLevel1) { ?>
                  ...
                  <div class="comment-view-level-1">
                      <div class="comment-view-author"><?= esc($commentLevel1->user_nickname); ?></div>
                      <div class="comment-view-date"><?= esc($commentLevel1->date); ?></div>
                      <div class="comment-view-comment"><?= esc($commentLevel1->content); ?></div>
                      <?php if($commentLevel1->status === 'Attente') { ?>
                        <form class="comment-view-choice" method="post" action="">
                          <input type="hidden" name="status" value="">
                          <input type="hidden" name="id" value="<?= esc($commentLevel1->id); ?>">
                  ...
                  <div class="comment-view-level-2">
                      <div class="comment-view-author"><?= esc($commentLevel2->user_nickname); ?></div>
                      <div class="comment-view-date"><?= esc($commentLevel2->date); ?></div>
                      <div class="comment-view-comment"><?= esc($commentLevel2->content); ?></div>
                      <?php if($commentLevel2->status === 'Attente') { ?>
                        <form class="comment-view-choice" method="post" action="">
                          <input type="hidden" name="status" value="">
                          <input type="hidden" name="id" value="<?= esc($commentLevel2->id); ?>">
                          <input type="submit" name="submit" value="Valider" class="btn btn-success submit-button"><input type="submit" name="submit" value="Refuser" class="btn btn-danger submit-button">
                        </form>
